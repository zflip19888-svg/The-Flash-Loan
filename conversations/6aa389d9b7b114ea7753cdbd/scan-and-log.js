 ...
 *
 * Fixes from anomaly reports #93/#100:
 *   - isPhantom now = isPhantomUniversal (universal guard, not stablecoin-only)
 *   - isPhantomUniversal + spreadPctOfLoan written to JSONL output
 *   - Aave fee 0.05% (0.0005)
 */
...
const SCAN_PAIRS = [
  { from: TOKENS.WETH,   to: TOKENS.USDC,   amount: 10,     label: 'WETH to USDC (borrow WETH)' },
  { from: TOKENS.WETH,   to: TOKENS.USDC,   amount: 15,     label: 'WETH to USDC (15 ETH)' },
  { from: TOKENS.WETH,   to: TOKENS.WMATIC, amount: 10,     label: 'WETH to WMATIC (via WETH borrow)' },
  { from: TOKENS.WMATIC, to: TOKENS.USDC,   amount: 20000,  label: 'WMATIC to USDC' },
  { from: TOKENS.WMATIC, to: TOKENS.USDC,   amount: 50000,  label: 'WMATIC to USDC (50K)' },
];
...
async function main() {
  ...
  console.log("MATIC $" + (maticUsd?.toFixed(4) ?? "?") + " | ETH $" + (ethUsd?.toFixed(0) ?? "?") + " | Gas " + gasPriceGwei.toFixed(1) + " Gwei | tx cost " + (gasCostUsd ? "$" + gasCostUsd.toFixed(3) : "?"));
  ...
  for (const pair of SCAN_PAIRS) {
    ...
    // Universal phantom guard: spread > 5% of loan notional
    ...
    // FIX #93/#100: isPhantom = universal guard (not stablecoin-only)
    ...
    console.log(pairLabel + " " + amount + " " + from.symbol + " -- " + signal + " spread=" + spreadPct?.toFixed(4) + "% net=$" + netProfitAdj?.toFixed(2) + " slip=" + slippageEst?.toFixed(1) + "%");
  ...
  fs.writeFileSync(logFile, jsonlLines.join("\n") + "\n");
  ...
  console.log("\n---JSONL_START---");
  console.log(jsonlLines.join("\n"));
  console.log("---JSONL_END---");
}
