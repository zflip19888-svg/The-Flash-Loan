/**
 * Live spread scanner — JSONL output for daily opportunity log
 * Based on scripts/live-spread-scan.js from The-Flash-Loan repo
 */

require("dotenv").config({ path: "/app/.agents/.env" });
...
async function main() {
  ...
  console.error(`\n  LIVE SPREAD SCANNER — Polygon Mainnet`);
  console.error(`  Block: #${blockNumber}   |   ${ts}`);
  console.error(`  MATIC $${maticUsd?.toFixed(4) ?? "?"}  ETH $${ethUsd?.toFixed(0) ?? "?"}  BTC $${btcUsd?.toFixed(0) ?? "?"}  Gas ${gasPriceGwei.toFixed(1)} Gwei`);
  console.error("");
  ...
  for (const pair of SCAN_PAIRS) {
    ...
    if (maxDepth < MIN_EITHER_RESERVE_USD) {
      ...
      results.push(entry);
      console.error(`  ${pairLabel}: ⚫ NO LIQUIDITY`);
      continue;
    }
    ...
    if (!qsOut && !ssOut) {
      ...
      results.push(entry);
      console.error(`  ${pairLabel}: ⚫ QUOTE FAILED`);
      continue;
    }
    ...
    if (spreadPct !== null && spreadPct < MIN_SPREAD_PCT) {
      ...
      results.push(entry);
      console.error(`  ${pairLabel}: ⚪ FLAT (${spreadPct.toFixed(4)}%)`);
      continue;
    }
    ...
    if (qsF !== null && ssF !== null) {
      ...
      if (priceOut) spreadUsd = spreadUnits * priceOut;
    }
    ...
    if (isPhantomUniversal) {
      signal = "👻 PHANTOM%"; signalShort = "PHANTOM";
    } else if (isPhantom) {
      signal = "👻 PHANTOM"; signalShort = "PHANTOM";
    } else if (isFlat) {
      signal = "⚪ FLAT"; signalShort = "FLAT";
    } else if (depthOk && netProfitAdj !== null && netProfitAdj > 2) {
      signal = "🟢 EXECUTE"; signalShort = "EXECUTE";
    } else if (!depthOk && netProfitAdj !== null && netProfitAdj > 2) {
      signal = "🟡 LOW DEPTH"; signalShort = "LOW_DEPTH";
    } else if (netProfitAdj !== null && netProfitAdj > 0) {
      signal = "🟡 MARGINAL"; signalShort = "MARGINAL";
    } else if (netProfitAdj !== null) {
      signal = "🔴 UNPROFITABLE"; signalShort = "UNPROFITABLE";
    } else if (spreadPct && spreadPct > 1) {
      signal = depthOk ? "🟡 CHECK" : "🟡 LOW DEPTH"; signalShort = depthOk ? "CHECK" : "LOW_DEPTH";
    } else {
      signal = "—"; signalShort = "NONE";
    }
    ...
    const entry = {
      ...
      isPhantom,
      isPhantomUniversal,
      phantom: isPhantom || isPhantomUniversal,
      aaveFeeUsd,
      loanNotionalUsd,
    ...
    results.push(entry);
    console.error(`  ${pairLabel}: ${signal}  spread ${spreadPct?.toFixed(2)}%  $${spreadUsd?.toFixed(2)}  net ~$${netProfitAdj?.toFixed(2)}`);
  }

  // Write JSONL
  const today = process.env.SCAN_DATE || new Date().toLocaleDateString('en-CA', { timeZone: 'America/Chicago' });
  const logPath = `logs/opportunities-${today}.jsonl`;
  fs.mkdirSync("logs", { recursive: true });

  // Append to file (so multiple scan runs per day accumulate)
  const jsonl = results.map(r => JSON.stringify(r)).join("\n") + "\n";
  fs.appendFileSync(logPath, jsonl);

  console.error(`\n  ✅ Wrote ${results.length} entries to ${logPath}`);
  console.error(`  Signals: ${results.map(r => r.signal).join(", ")}`);

  // Print summary for the automation to capture
  ...
  const phantom     = results.filter(r => r.signal === "👻 PHANTOM%" || r.signal === "👻 PHANTOM");
  console.log("\n=== SCAN SUMMARY ===");
  console.log(`Block: #${blockNumber} | ${ts}`);
  ...
  console.log(`🟢 EXECUTE: ${executable.length} | 🟡 LOW DEPTH: ${lowDepth.length} | 🟡 MARGINAL: ${marginal.length} | 👻 PHANTOM: ${phantom.length}`);
  if (executable.length) {
    console.log("Best opportunities:");
    for (const r of executable.sort((a,b) => (b.netProfitAdj ?? 0) - (a.netProfitAdj ?? 0))) {
      console.log(`  ${r.pair} ${r.amount} — spread ${r.spreadPct?.toFixed(2)}% — net ~$${r.netProfitAdj?.toFixed(2)} (slip ${r.slippageEst?.toFixed(1)}%)`);
    }
  }
}
